...
<!--                <el-table-column prop="name" label="用户名"></el-table-column>-->
                ...
                <el-table-column label="状态" align="center">
                    <template slot-scope="scope">
                        <el-tag
                            :type="scope.row.status===0?'默认':'不默认'"
                        >{{scope.row.status===0?'默认':'不默认'}}</el-tag>
                    </template>
                </el-table-column>
...
export default {
    ...
    methods: {
        // 获取 easy-mock 的模拟数据
        getData() {
            request.fetchPost('/group/select').then((res)=>{
        ...
        handleDelete(index, row) {
            ...
            request.fetchPost('/group/delete',{activityId:row.activityId}).then(function (res) {
                 this.getData()
